  ...
  return (
    <Link 
      ...
      className={`block rounded-xl overflow-hidden transition-all duration-300 hover-lift bg-white ${
        featured ? 'border-2 border-foodly-accent/30' : 'border border-foodly-200'
      }`}
    >
      <div className="relative">
        ...
        <div className="p-4">
          <div className="flex justify-between items-start mb-2">
            <h3 className="font-bold text-lg text-foodly-900 line-clamp-1">{name}</h3>
          ...
          <div className="flex flex-wrap items-center gap-y-2 gap-x-3 text-xs text-foodly-700">
            ...
            <div className={`flex items-center ${deliveryFee === 'Free Delivery' ? 'text-foodly-accent font-medium' : ''}`}>
              {deliveryFee === 'Free Delivery' ? 'Free Delivery' : `Delivery: ${deliveryFee}`}
            </div>
          </div>
        </div>
      </div>
