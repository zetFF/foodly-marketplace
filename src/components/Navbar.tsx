...
import { Search, ShoppingBag, Menu, X, User, Heart, Bell, Home, Utensils, Tag, Info, LogIn } from 'lucide-react';
            ...
            <div className="hidden lg:block">
              <NavigationMenu>
                <NavigationMenuList>
                  ...
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className={isActive('/restaurants') ? "bg-accent/50 text-accent-foreground font-medium" : ""}>
                  ...
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className={isActive('/categories') ? "bg-accent/50 text-accent-foreground font-medium" : ""}>
                  ...
                  <NavigationMenuItem>
                    <Link to="/customer-features">
                      <NavigationMenuLink className={cn(
                        navigationMenuTriggerStyle(),
                        isActive('/customer-features') ? "bg-accent/50 text-accent-foreground font-medium" : ""
                      )}>
                        Features
                      </NavigationMenuLink>
                    </Link>
                  ...
                  <NavigationMenuItem>
                    <Link to="/about">
                      <NavigationMenuLink className={cn(
                        navigationMenuTriggerStyle(),
                        isActive('/about') ? "bg-accent/50 text-accent-foreground font-medium" : ""
                      )}>
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="rounded-full hover:bg-foodly-100">
              ...
              {isLoggedIn && (
                <>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" className="rounded-full hover:bg-foodly-100 relative">
                          <Bell className="h-5 w-5 text-foodly-700" />
                          <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 flex items-center justify-center bg-foodly-secondary">2</Badge>
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>Notifications</p>
              ...
              {isLoggedIn ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="rounded-full w-10 h-10 p-0 border-2 border-foodly-accent">
                  ...
                  <DropdownMenuContent align="end" className="w-60 p-2">
                    ...
                    <DropdownMenuItem>
                      <Link to="/profile?tab=orders" className="w-full flex items-center">
                        <ShoppingBag className="h-4 w-4 mr-2" />
                        Orders
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem>
                      <Link to="/profile?tab=wishlist" className="w-full flex items-center">
                        <Heart className="h-4 w-4 mr-2" />
                        Wishlist
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem>
                      <Link to="/profile?tab=settings" className="w-full flex items-center">
          ...
          <div className="md:hidden fixed inset-0 z-10 bg-white animate-fade-in pt-16">
            <div className="foodly-container h-full py-4 overflow-auto">
              <div className="space-y-4 mb-8">
                ...
                <div className="py-2">
                  <div className="px-4 mb-2 text-sm font-semibold text-foodly-500">Restaurants</div>
                  <Link 
                    to="/restaurants" 
                ...
                <div className="py-2">
                  <div className="px-4 mb-2 text-sm font-semibold text-foodly-500">Categories</div>
                  <div className="grid grid-cols-2 gap-1">
                    {['Pizza', 'Burgers', 'Sushi', 'Italian', 'Chinese', 'Mexican'].map((category) => (
                      <Link 
                        key={category}
                        to={`/categories/${category.toLowerCase()}`} 
              ...
              {isLoggedIn && (
                <div className="border-t border-foodly-100 pt-4 space-y-4">
                  <div className="px-4 mb-2 text-sm font-semibold text-foodly-500">Your Account</div>
                  <Link 
                    to="/profile" 
              ...
              <div className="flex items-center space-x-4 py-4 mt-4 border-t border-foodly-100">
                ...
                <Button variant="ghost" size="icon" className="rounded-full hover:bg-foodly-100 relative">
                  <ShoppingBag className="h-5 w-5 text-foodly-700" />
                  <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 flex items-center justify-center bg-foodly-accent">{cartItemCount}</Badge>
                </Button>
